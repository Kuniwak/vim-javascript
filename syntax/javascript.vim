...
syntax region  jsLineComment    start=+^\s*\/\/+ skip=+\n\s*\/\/+ end=+$+ keepend contains=jsCommentTodo,@Spell,jsDocMarkdownCode fold
...
if !exists("javascript_ignore_javaScriptdoc")
  ...
  syntax region jsDocComment matchgroup=javaScriptComment start="/\*\*\s*"  end="\*/" contains=jsDocTag,jsDocInlineTag,javaScriptCommentTodo,javaScriptCvsTag,@javaScriptHtml,@Spell,jsDocHTMLTag,jsDocMarkdownCode fold keepend
  ...
  syntax match  jsDocTag       contained "@" nextgroup=jsDocTypeParamDescTagNames,jsDocTypeDescTagNames,jsDocTypeTagNames,jsDocDescTagNames,jsDocMarkerTagNames,jsDocSuppressTagNames,jsDocAuthorTagNames
  syntax region jsDocInlineTag contained matchgroup=jsDocInlineCurlyBrackets start="{@" end="}" contains=jsDocInlineTagNames
  ...
  syntax keyword jsDocTypeParamDescTagNames   contained param nextgroup=jsDocTypeParamDescTagType skipwhite skipnl
  syntax keyword jsDocTypeDescTagNames        contained define enum return throws nextgroup=jsDocTypeDesc skipwhite skipnl
  syntax keyword jsDocTypeTagNames            contained extends implements this type typedef nextgroup=jsDocType skipwhite skipnl
  syntax keyword jsDocDescTagNames            contained see deprecated fileoverview license preserve template nextgroup=jsDocDesc skipwhite skipnl
  syntax keyword jsDocMarkerTagNames          contained const constructor interface inheritDoc expose dict private protected struct nosideeffects override preserveTry skipwhite
  syntax keyword jsDocAuthorTagNames          contained author nextgroup=jsDocAuthorContent skipwhite skipnl
  syntax keyword jsDocSuppressTagNames        contained suppress nextgroup=jsDocSuppressFlag skipwhite skipnl
  syntax keyword jsDocInlineTagNames          contained code link nextgroup=jsDocInlineTagContent skipwhite skipnl

  syntax match   jsDocHTMLTag                 contained "<[^>]\+>"

  " detect valied JavaScript object name path such as: foo.Bar or foo.Bar#someMethod
  syntax match   jsDocNameContent             contained "\%(\w\|_\|\$\)\(\%(\w\|\d\|_\|\$\|\.\|#\)*\%(\w\|\d\|_\|\$\)\)\?" contains=jsDocPredefinedObjects
  " detect type expression operators
  " See: https://developers.google.com/closure/compiler/docs/js-for-compiler#types
  syntax match   jsDocTypeOperator            contained "\%(|\|=\|!\|?\|\*\|\.\.\.\|,\|(\|)\|:\)"
  " ActionScript3 like generics
  syntax region  jsDocGenerics                contained matchgroup=jsDocTypeOperator start="\.<" end=">" contains=jsDocNameContent,jsDocTypeOperator,jsDocGenerics
  syntax region  jsDocRecordType              contained matchgroup=jsDocTypeOperator start="{" end="}" contains=jsDocRecordKey
  syntax match   jsDocRecordKey               contained "[^:]*" nextgroup=jsDocRecordPanctuator skipwhite skipnl
  syntax match   jsDocRecordPanctuator        contained ":" nextgroup=jsDocNameContent,jsDocTypeOperator,jsDocGenerics,jsDocRecordType skipwhite skipnl

  " TODO: implement function expression

  " detect correct author tag statement
  syntax match   jsDocAuthorContent           contained "[^@]\+@\S\+\s([^)]\+)"
  syntax match   jsDocInlineTagContent        contained "[^}]\+"
  syntax region  jsDocMarkdownCode            contained matchgroup=jsDocBackQuotes start="`" end="`" contains=jsDocMarkdownCodeContent
  syntax region  jsDocSymbolName              contained start="{" end="}" contains=jsDocNameContent
  syntax region  jsDocType                    contained matchgroup=jsDocCurlyBrackets start="{" end="}" contains=jsDocNameContent,jsDocTypeOperator,jsDocGenerics,jsDocRecordType
  syntax region  jsDocTypeDesc                contained matchgroup=jsDocCurlyBrackets start="{" end="}" contains=jsDocNameContent,jsDocTypeOperator,jsDocGenerics,jsDocRecordType nextgroup=jsDocDesc
  syntax region  jsDocTypeParamDescTagType    contained matchgroup=jsDocCurlyBrackets start="{" end="}" contains=jsDocNameContent,jsDocTypeOperator,jsDocGenerics,jsDocRecordType nextgroup=jsDocTypeParamDescTagParam skipwhite skipnl
  syntax match   jsDocTypeParamDescTagParam   contained "\*\?\s*\%(\w\|_\|\$\)\%(\w\|\d\|_\|\$\)*" nextgroup=jsDocDesc skipwhite skipnl
  syntax match   jsDocDesc                    contained ".*\(\s\|\n\)" contains=jsDocInlineTag,jsDocMarkdownCode

  " suppress tag flags
  " See: https://code.google.com/p/closure-compiler/wiki/Warnings#Warnings_Categories
  syntax region  jsDocSuppressFlag            contained matchgroup=jsDocCurlyBrackets start="{" end="}" contains=jsDocSuppressFlagContent
  syntax keyword jsDocSuppressFlagContent     contained accessControls ambiguousFunctionDecl checkDebuggerStatement checkRegExp checkTypes checkVars const constantProperty deprecated duplicate es5Strict externsValidation fileoverviewTags globalThis internetExplorerChecks invalidCasts missingProperties nonStandardJsDocs strictModuleDepCheck suspiciousCode undefinedNames undefinedVars unknownDefines uselessCode visibility
